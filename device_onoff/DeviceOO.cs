...
ref PropertyChangeParameters classInstallParams, int classInstallParamsSize);
    ...
    public sealed class DeviceSwitcher
    {
        ...
        public static void SetDeviceEnabled(string DeviceID, bool enable)
        {
            string classguid = null;
            try
            {
                ManagementObjectSearcher searcher = new ManagementObjectSearcher("root\\CIMV2", "SELECT * FROM Win32_PnPSignedDriver WHERE DeviceID = \"" + DeviceID.Replace(@"\", @"\\") + "\"");
                foreach (ManagementObject queryObj in searcher.Get())
                    classguid = queryObj["ClassGuid"].ToString();
            }
            catch { };

            if (string.IsNullOrEmpty(classguid)) return;
            Guid classGUID = new Guid(classguid);
            SetDeviceEnabled(classGUID, DeviceID, enable);
        }

        public static void SetDeviceEnabled(Guid ClassGuid, string DeviceID, bool on_or_off)
        {
            ...
            try
            {
                ...
                int index = GetIndexOfInstance(diSetHandle, diData, DeviceID);
                // Disable...
                EnableDevice(diSetHandle, diData[index], on_or_off);
            }
            finally
            {
                if (diSetHandle != null)
